...
public class Organisation implements Estado, Antecessor {
	 ...
	 * https://www.wolframalpha.com/ for exponential model The given inputs were:
	 * {1, 2}, {2, 7}, {3, 47}, {4, 501}, {5, 7174} Which were taken by observation
	 * using 1 to 5 goals for goal tree with no constraints (i.e. only workloads
	 * with effort less than the grain and sum of workloads not greater than max in
	 * order to allow all combinations) The obtained equation is: 0.0121226 *
	 * Math.exp(2.65818 * nGoals); The result is not good for 1 ~ 3 goals but for 4 
	 * goals and more has error less than 0.2% (max tested was 6) 
	 * 
	 * @return an integer with approximated number organisations that will be created
	 * in the worst case
	 */
